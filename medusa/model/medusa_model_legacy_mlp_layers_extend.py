...
class MLPTokenClassifier(nn.Module):
    ...
    """
    def __init__(self, hidden_dim, hidden_layers=[128, 64], vocab_size=32000, num_heads=4, extra_layers_per_head=None):
        super().__init__()
...
class MedusaModel(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            extra_layers_per_head (list, optional): Number of extra ResBlock layers for each head. Defaults to list(range(medusa_num_heads)).
        """
        super().__init__()
        ...
        self.use_mlp = use_mlp
        if extra_layers_per_head is None:
            extra_layers_per_head = list(range(medusa_num_heads))
        self.extra_layers_per_head = extra_layers_per_head
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        Args:
            ...
            extra_layers_per_head (list, optional): Number of extra ResBlock layers for each head.
        ...
        if extra_layers_per_head is None and hasattr(medusa_config, 'extra_layers_per_head'):
            extra_layers_per_head = medusa_config.extra_layers_per_head
