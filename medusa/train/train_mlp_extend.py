...
import os
from medusa.model.medusa_model_legacy_mlp_layers_extend import MedusaModel, MedusaConfig
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    use_mlp: bool = field(
        default=True,
        metadata={"help": "Whether to use MLP architecture for Medusa heads."},
    )
    # mlp_hidden_layers: list = field(
    #     default_factory=lambda: [1024, 512],
    #     metadata={"help": "Hidden layer sizes for MLP backbone."},
    # )
    # extra_layers_per_head: Optional[list] = field(
    #     default=None,
    #     metadata={"help": "Number of extra ResBlock layers for each head. If None, defaults to list(range(medusa_num_heads))."},
    # )
...
def train():
    ...
    # if training_args.extra_layers_per_head is not None:
    #     # Parse comma-separated string of 0/1 or False/True values
    #     grounded_str = training_args.extra_layers_per_head.strip()
    #     grounded_parts = [s.strip() for s in grounded_str.split(',')]
    #     grounded_heads = []
    #     for part in grounded_parts:
    #         if part.lower() in ['1', 'true']:
    #             extra_layers_per_head.append(1)
    #         elif part.lower() in ['0', 'false']:
    #             extra_layers_per_head.append(0)
    #         else:
    #             raise ValueError(f"Invalid value in grounded_heads: {part}. Expected '0', '1', 'True', or 'False'.")
        
    #     # Validate length matches medusa_num_heads
    #     if len(extra_layers_per_head) != training_args.medusa_num_heads:
    #         raise ValueError(
    #             f"Length of extra_layers_per_head ({len(extra_layers_per_head)}) must match medusa_num_heads ({training_args.medusa_num_heads})"
    #         )
    ...
    medusa_lm_head = MedusaModel(
        ...
        use_mlp=training_args.use_mlp,
        mlp_hidden_layers=[1024, 512],
        extra_layers_per_head=[0,0,1,1,1],
    )

    # Format output dir
    training_args.output_dir = f"{training_args.output_dir}_medusa_mlp_extn_{model_args.model_name_or_path.split('/')[-1]}_medusa_{training_args.medusa_num_heads}_lr_{training_args.learning_rate}_layers_{training_args.medusa_num_layers}"
