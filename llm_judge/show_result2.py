...
def display_result_single(args):
    ...
    # current_subject = subjects[(question_id - 80) // 10]
    ...
    if "mt_bench" in args.bench_name:
        ...
        print("\n########## Quality per Subject ##########")
        df["subject"] = df["question_id"].apply(lambda x: subjects[(x - 81) // 10])
        df_subject = df.groupby(["model", "subject"]).mean()["score"].unstack()
        print(df_subject)
